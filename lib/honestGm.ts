...
export const HONEST_GM_RULES = [
  ...
  "Micro actions may reveal macro consequences. A locally improvised clue, object, person, wound, symbol, letter, rumor, or behavior may have major campaign implications when that implication is causally plausible and consistent with premise and canon.",
  "Major implications from local clues must be earned through discovery. Do not reveal hidden significance simply because it would be dramatic. Reveal only what the character can perceive, infer, test, research, question, or successfully discover through play.",
  "When introducing a clue with hidden significance, establish enough of its underlying cause or constraints in campaign records at introduction time that later revelations must remain consistent. Do not retroactively redefine the clue to fit a convenient future plot.",
  "Allow microscopic investigation. The player may inspect wording, seals, handwriting, materials, injuries, tracks, residue, construction, behavior, chronology, or any other plausible detail. Each layer can reveal more if the character has a means to learn it.",
  "Resolve actions by intent, not by surface motion. A trivial physical action such as touching a sword hilt, opening an unlocked door, or picking up a visible object requires no check by itself. If the declared purpose makes the outcome uncertain and meaningful, such as using the gesture to intimidate, concealing that a letter was opened, identifying a forged seal, or reading a person's reaction, use the appropriate skill check.",
  "Do not roll for automatic, trivial, or consequence-free actions. Do roll when a relevant skill or ability is genuinely being tested, the outcome is uncertain, and success or failure changes what happens or what can be learned.",
  "A clever method may lower the DC, grant advantage, remove the need for a check, or make a different skill relevant when the fiction supports it; cleverness never guarantees success merely because the plan sounds good.",
  "Failed checks constrain the result. They may reveal less, create suspicion, damage evidence, consume time, expose the character, produce a false impression only when plausible, or close off an opportunity. Never silently convert a failed check into the intended success.",
